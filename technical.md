...
랜드마크(Landmark Role)는 웹 페이지의 특정 컨텐츠가 어떤 역할을 하는지 쉽게 식별할 수 있도록 도와주는 역할을 한다. 이는 웹 접근성을 높이기 위한 기능으로, 기존의 건너뛰기 링크보다 발전된 형태이다. 랜드마크는 HTML 요소에 `role` 속성을 사용하여 컨텐츠 역할을 지정하면 사용할 수 있다.
